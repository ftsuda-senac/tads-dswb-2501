...
public class JwtService {
    ...
    public String gerarJwt(UsuarioSistema usuario) {
        ...
        Instant expiry = now.plus(intervaloExpiracao, ChronoUnit.MINUTES);
        ...
        // Define o cabeçalho JWS (opcional se o encoder tiver um padrão, mas bom explicitar)
        // Usamos MacAlgorithm.HS256 que corresponde ao HmacSHA256
    ...
    public UsuarioSistema obterUsuarioDoToken(String token) {
        try {
            ...
            UsuarioSistema usuario = new UsuarioSistema(jwt.getClaimAsString("name"), username, null, permissoes);
            return usuario;
    ...
    // Verifica se um objeto Jwt decodificado é válido (não expirado)
    private boolean isTokenValido(Jwt jwt) {
        Instant expiresAt = jwt.getExpiresAt();
        // Verifica se a data de expiração existe e é depois do momento atual
